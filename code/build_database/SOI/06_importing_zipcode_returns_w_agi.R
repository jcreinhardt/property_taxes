...
# Aggregate to one observation per zipcode-year-agi_stub
zipcode_data_agi <- zipcode_data_agi |>
    select(-any_of(c("state", "statefips"))) |>
    group_by(year, zipcode, agi_stub) |>
    summarise(
        across(where(is.numeric) & !matches("^(year|zipcode|agi_stub)$"), ~ sum(.x, na.rm = TRUE)),
        .groups = "drop"
    )

# Counts within each agi stub by year
zipcode_data_agi |>
...
# Compare zipcode aggregates to state aggregates via HUD crosswalk (by AGI stub)
hud_crosswalk <- dbGetQuery(con, "select * from hud_zip_to_county") |>
    mutate(year = year - 1) |> # Since you file the year after the tax year 
    expand_grid(agi_stub = 1:6)

zip_to_county <- zipcode_data_agi |>
    left_join(hud_crosswalk, by = c("zipcode", "year", "agi_stub")) |>
    filter(!is.na(county) & county != 99999) |> # Not sure what the 99999 counties are
    group_by(county, year, agi_stub) |>
    summarise(
        across(where(is.numeric), ~ sum(.x * res_ratio, na.rm = TRUE)),
        .groups = "drop"
    )
...
state_returns <- dbGetQuery(con, "select * from returns_state") |>
    ...
    mutate(agi_stub = case_when(
        agi_stub %in% c(2, 3) ~ 1,
        agi_stub %in% c(4, 5, 6, 7) ~ agi_stub - 2,
        agi_stub >= 8 ~ 6,
        TRUE ~ agi_stub
    )) |>
...
zip_v_state <- zip_to_state |>
    ...
    left_join(state_returns, by = c("state", "year", "agi_stub"), suffix = c("_zip", "_state"))
